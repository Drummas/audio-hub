...
DEFAULT_CONFIG = {
    ...
    "auto_model_enabled": os.getenv("AUTO_MODEL_ENABLED", "True") not in ("false", "False", "0"),
...
async def model_speedtest():
    ...
    if CONFIG["auto_model_enabled"] and not CONFIG.get("whisper_model_override"):
...
async def transcribe_audio(
    ...
    elapsed = time.time() - start
    logger.info(
        f"transcribe_done filename={file.filename} status_code={resp.status_code} elapsed={elapsed:.2f}"
    )
...
async def translate_audio(
    ...
    target_language: str = Form(CONFIG["default_translation_language"]),
):
    with tempfile.NamedTemporaryFile(delete=False) as tmp:
        shutil.copyfileobj(file.file, tmp)
        path = tmp.name
    ...
    start = time.time()
    logger.info(f"translate_start filename={file.filename} language={target_language}")
    ...
    elapsed = time.time() - start
    logger.info(
        f"translate_done filename={file.filename} status_code={resp.status_code} elapsed={elapsed:.2f}"
    )
...
def process_batch(batch_id, file_ids, task, language):
    ...
    logger.info(
        json.dumps(
            {"event": "batch_complete", "batch_id": batch_id, "total": total}
        )
    )
...
async def ui_files(request: Request):
    meta = load_metadata()
    return templates.TemplateResponse("files.html", {"request": request, "files": meta})
...
async def ui_performance(request: Request):
    # reuse system page for now
    return await ui_system(request)